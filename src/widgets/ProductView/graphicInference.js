...
export default function inferGraphic(type, value){
  ...
  if(type === 'cluster_type'){
    let core = 'gke';
    if(value.includes("gke"))
      core = 'gke';
    else if(value.includes("eks"))
      core = 'eks';
    else if(value.includes("aks"))
      core = 'aks';
    return {
      graphicType: 'image',
  ...
  else if(type === 'cluster_features') {
    if (value.includes("kubernetes")) {
      return {
        graphicType: "image",
  ...
  else if(type === 'software_and_services') {
    let core = null;
    if (value.includes("prometheus"))
